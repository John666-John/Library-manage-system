...
from PyQt5.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QLabel, QLineEdit,
                             ...
                             QFormLayout, QHeaderView, QApplication)
...
class BookManagementTab(QWidget):
    ...
    def delete_book(self):
        ...
        if QMessageBox.question(self, "确认", "确定要删除选中的图书吗？\n删除后不可恢复，相关借阅记录不受影响",
...
class BookDialog(QWidget):
    ...
    def accept(self):
        if not all([self.id_edit.text().strip(), self.title_edit.text().strip(), self.author_edit.text().strip()]):
            QMessageBox.warning(self, "警告", "带*的字段不能为空")
            return
        self.result = True
        self.close()
